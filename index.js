...
client.once("ready", async () => {
    ...
    const commands = [
        ...
        new SlashCommandBuilder().setName("clans").setDescription("Post clan application info.")
    ].map(cmd => cmd.toJSON());
...
client.on("interactionCreate", async (interaction) => {
    ...
    if (commandName === "remind") {
        ...
        const embed = {
            color: 0xff0000,
            title: "⏰ Reminder",
            description: "We are still awaiting a response from you. Please respond at your earliest convenience.\n\nLost Family Team",
            timestamp: new Date()
        };

        await interaction.editReply("✅ Reminder sent.");
    ...
    if (commandName === "clans") {
        const embed = {
            ...
            description: `To apply for a Lost Family clan, please go to <#${process.env.TICKET_CHANNEL_ID}> and submit your request.`,
            footer: {
                text: `Lost Family Team • Today at ${new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`
            },
            timestamp: new Date()
        };
