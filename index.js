...
// Validate users and schedule reminders. Removes unknown user IDs.
async function rescheduleReminders() {
    ...
    for (const userId of Object.keys(reminders)) {
        // Validate user exists before scheduling; remove if unknown
        try {
        ...
        for (const reminder of reminders[userId]) {
            ...
            if (delay <= 0) {
                // deliver immediately but don't await to avoid blocking startup
...
client.once("ready", async () => {
    ...
    await rescheduleReminders(); // 🔄 Auto-reschedule reminders on startup (validates and cleans invalid IDs)
});
